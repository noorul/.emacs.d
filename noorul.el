...
(defvar aws-sso-profiles-buffer nil
  "Buffer for storing AWS profiles output.")

(defvar aws-sso-login-buffer nil
  "Buffer for storing AWS SSO login output.")

(defvar aws-sso-selected-profile nil
  "Selected AWS profile for SSO login.")

(defun aws-sso-check-aws-cli ()
  "Check if AWS CLI is available in the system."
  (unless (executable-find "aws")
    (error "AWS CLI is not installed or not in PATH. Please install it first")))

(defun aws-sso-handle-process-error (process signal)
  "Handle process errors for AWS commands."
  (let ((err-msg (with-current-buffer (process-buffer process)
                   (buffer-string))))
    (kill-buffer (process-buffer process))
    (error "AWS command failed: %s\nError: %s" signal err-msg)))

(defun aws-sso-login-sentinel (process event)
  "Handle AWS SSO login process completion."
  (cond
   ((string= event "finished\n")
    (message "AWS SSO login completed for profile: %s" aws-sso-selected-profile)
    (kill-buffer aws-sso-login-buffer))
   ((string-prefix-p "exited abnormally" event)
    (aws-sso-handle-process-error process event))))

(defun aws-sso-profiles-sentinel (process event)
  "Handle AWS profiles listing process completion."
  (cond
   ((string= event "finished\n")
    (let ((profiles (with-current-buffer aws-sso-profiles-buffer
                     (split-string (buffer-string) "\n" t))))
      (if (null profiles)
          (error "No AWS profiles found. Please configure at least one profile")
        (setq aws-sso-selected-profile
              (completing-read "Select AWS profile: " profiles nil t))
        (kill-buffer aws-sso-profiles-buffer)

        ;; Start the login process
        (setq aws-sso-login-buffer (generate-new-buffer "*aws-sso-login*"))
        (set-process-sentinel
         (start-process "aws-sso-login" aws-sso-login-buffer
                       "aws" "sso" "login" "--profile" aws-sso-selected-profile)
         'aws-sso-login-sentinel))))
   ((string-prefix-p "exited abnormally" event)
    (aws-sso-handle-process-error process event))))

(defun aws-sso-login ()
  "Interactive AWS SSO login with profile selection."
  (interactive)
  (condition-case err
      (progn
        (aws-sso-check-aws-cli)
        (setq aws-sso-profiles-buffer (generate-new-buffer "*aws-profiles*"))
        (set-process-sentinel
         (start-process "aws-list-profiles" aws-sso-profiles-buffer
                       "aws" "configure" "list-profiles")
         'aws-sso-profiles-sentinel))
    (error
     (message "Error in aws-sso-login: %s" (error-message-string err))
...
(use-package "ledger-mode"
  ...
  :config
  (progn
    (ledger-reports-add "monthly budget" "%(binary) -f %(ledger-file) -y '%a %e %b %Y' --budget -M -p \"this month\" register Expenses Assets:LivretA")
    (ledger-reports-add "monthly unbudgeted" "%(binary) -f %(ledger-file) -y '%a %e %b %Y'  --unbudgeted -M -p \"this month\"  register Expenses Income")
    (ledger-reports-add "by Payee" "%(binary) -f %(ledger-file) -y '%a %e %b %Y' register -P")
    (ledger-reports-add "uncleared" "%(binary) -f %(ledger-file) -y '%a %e %b %Y' register -U")
    (ledger-reports-add "bal" "%(binary) -f %(ledger-file) -y '%a %e %b %Y' cleared")
    (ledger-reports-add "monthly expenses" "%(binary) -f %(ledger-file) --aux-date -y '%b %Y' -M -n reg Expenses")
    (ledger-reports-add "monthly expenses weekly" "%(binary) -f %(ledger-file) -y '%a %e %b %Y' -W -n reg Expenses -p \"this month\"")
    (ledger-reports-add "monthly expenses daily" "%(binary) -f %(ledger-file) -y '%a %e %b %Y' -D -n reg Expenses -p \"this month\"")
    (ledger-reports-add "net worth" "%(binary) -f %(ledger-file) -X INR balance ^Assets ^Liabilities")
    (ledger-reports-add "wallet" "%(binary) -f %(ledger-file) balance ^Assets:Wallet ^Assets:Sajida:Wallet")
    (ledger-reports-add "sajida" "%(binary) -f %(ledger-file) reg -p 'this month' Assets:Sajida:Wallet")
    (ledger-reports-add "current month expenses" "%(binary) -f %(ledger-file) --aux-date -p 'this month' -S -a reg Expenses")
    (ledger-reports-add "current month expenses with limit" "%(binary) -f %(ledger-file) --aux-date -p 'this month' -S -a reg Expenses --limit '(account !~ /AgtNew/) and (account !~ /Shifting/)'")
    ...
    (ledger-reports-add "credit card expenses report" "%(binary) -f %(ledger-file) reg Liabilities:Infinia --limit 'payee !~ /Credit Due/'"))
