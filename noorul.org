...
** Early Init
This section tangles to =~/.config/emacs/early-init.el=, which Emacs loads before
the package system and UI are initialised. Setting these values here prevents
the toolbar/scrollbar flash and reduces GC pressure during startup.

*** UI suppression
Disabling UI elements before the first frame is created avoids the visual flicker
of bars appearing and then being hidden.
...
*** GC and startup optimisations
Temporarily maximise GC threshold and disable expensive hooks during startup,
then restore sane values afterwards.
#+begin_src emacs-lisp :tangle "~/.config/emacs/early-init.el"
...
Tell package.el to initialise at startup using the available cache.
#+begin_src emacs-lisp :tangle "~/.config/emacs/early-init.el"
...
#+end_src
*** Navigate to emacs configuration with simple keybind
#+begin_src emacs-lisp
